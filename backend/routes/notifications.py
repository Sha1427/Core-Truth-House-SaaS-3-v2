            ...
            if message.get("type") == "ping":
                ...
                if notification_id:
                    ...
                    await websocket.send_json(
                        {"type": "read_confirmed", "notification_id": notification_id}
                    )
...
async def create_notification(data: NotificationCreate):
    ...
    if manager.is_user_online(data.user_id):
        unread_count = await db.notifications.count_documents(
            {"user_id": data.user_id, "is_read": False}
        )
...
async def mark_read(notification_id: str, user_id: Optional[str] = None):
    db = get_db()
...
async def notify_ai_job_complete(
    user_id: str,
    job_type: str,
    job_id: str,
    success: bool = True,
...
async def get_notification_stats(user_id: str):
    ...
    stats_by_category = await db.notifications.aggregate(pipeline).to_list(length=20)
    ...
    return {
        ...
        "unread": unread,
        "by_category": {
            s["_id"]: {"count": s["count"], "unread": s["unread"]}
            for s in stats_by_category
        },
    }