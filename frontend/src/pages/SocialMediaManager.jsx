...
export default function SocialMediaManager() {
  ...
  async function fetchAllData() {
    ...
    try {
      const [postsRes, calRes, platRes, anaRes] = await Promise.all([
        ...
        apiClient.get("/api/social/platforms", { workspace: false }),
  ...
  async function handleCreatePost() {
    try {
      ...
      await apiClient.post("/api/social/posts", postData);
      setShowModal(null);
      setFormData({});
      setMediaPreview([]);
      await fetchAllData();
  ...
  function renderCalendar() {
    ...
    for (let i = 0; i < firstDay; i += 1) {
      cells.push(
        <div
          ...
          style={{ background: "rgba(13,0,16,0.3)" }}
        />
      );
    }
    ...
    for (let day = 1; day <= daysInMonth; day += 1) {
      ...
      cells.push(
        <div
          key={day}
          className="h-28 rounded-xl p-2 transition-all border"
          style={{
        ...
        >
          <div className="flex items-center justify-between mb-1">
            ...
            {dayPosts.length > 0 && (
              <span className="text-xs text-[#4a3550]">{dayPosts.length}</span>
            )}
          </div>
          ...
          <div className="space-y-0.5 overflow-hidden">
            {dayPosts.slice(0, 3).map((post, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="truncate text-[#a08aaa]">
            ...
            {dayPosts.length > 3 && (
              <span className="text-xs text-[#4a3550] pl-1">
                +{dayPosts.length - 3}
              </span>
            )}
          </div>
  ...
  const filteredPosts = useMemo(
    () => posts.filter((p) => filterPlatform === "all" || p.platform === filterPlatform),
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0 flex-1 pl-10 md:pl-0">
                <h1 className="font-bold text-white text-lg md:text-xl flex items-center gap-2">
                  <Share2 size={20} className="text-[#e04e35] flex-shrink-0" />
              ...
              <button
                ...
                style={{
                  ...
                  boxShadow: "0 4px 16px rgba(224,78,53,0.3)",
                }}
              >
                ...
                <>
                  ...
                  <div className="grid grid-cols-7 gap-1 mb-1">
                    {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
                      <div
                        key={d}
                        className="text-center text-xs font-medium text-[#4a3550] py-2"
                      >
                        {d}
                      </div>
                    ))}
                              ...
                              {post.hashtags && post.hashtags.length > 0 && (
                                <div className="flex flex-wrap gap-1.5 mb-2">
                                  {post.hashtags.slice(0, 5).map((tag, i) => (
                                    <span
                                      key={i}
                                      className="text-xs px-2 py-0.5 rounded-full bg-[rgba(224,78,53,0.1)] text-[#e04e35]"
                                    >
                                      #{tag}
                                    </span>
                                  ))}
          ...
          >
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
                ...
                <button
                  onClick={() => setShowModal(null)}
                  className="text-[#4a3550] hover:text-white"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                <div>
                  <label className="text-xs text-[#763b5b] font-medium uppercase tracking-widest mb-1.5 block">
                    Platform
                  </label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                    {Object.entries(PLATFORM_CFG).map(([id, cfg]) => {
                      ...
                      return (
                        <button
                          key={id}
                          onClick={() => setFormData((d) => ({ ...d, platform: id }))}
                ...
                <div
                  className="rounded-2xl border border-[rgba(224,78,53,0.15)] overflow-hidden"
                  style={{ background: "rgba(224,78,53,0.04)" }}
                >
                  <div className="px-4 py-3 flex items-center gap-2 border-b border-[rgba(224,78,53,0.1)]">
                    <Sparkles size={14} className="text-[#e04e35]" />
                    <span className="text-xs font-semibold text-[#e04e35]">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-xs text-[#763b5b] font-medium uppercase tracking-widest">
                      Content
                    </label>
                    <span
                      className="text-xs"
                      style={{ color: charCount > charLimit ? "#ef4444" : "#4a3550" }}
                    >
                      {charCount}/{charLimit}
                    </span>
                  </div>
                ...
                {formData.hashtags?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {formData.hashtags.map((tag, i) => (
                      <span
                        key={i}
                        className="text-xs px-2.5 py-1 rounded-full bg-[rgba(224,78,53,0.1)] text-[#e04e35] border border-[rgba(224,78,53,0.15)]"
                      >
                        #{tag}
                      </span>
                    ))}
                        ...
                        <div key={i} className="relative group">
                          {m.type === "video" ? (
                            <div className="w-20 h-20 rounded-xl bg-[#2b1040] border border-white/10 flex items-center justify-center">
                              ...
                              <span className="text-[8px] text-gray-400 absolute bottom-1 inset-x-0 text-center truncate px-1">
                                {m.name}
                              </span>
                            </div>
                          ) : (
              ...
              <div className="px-6 py-4 border-t border-white/5 flex justify-end gap-2">
                <button
                  onClick={() => setShowModal(null)}
                  className="text-xs px-4 py-2.5 rounded-xl border border-white/10 text-[#a08aaa]"
                >
                  Cancel
                </button>
