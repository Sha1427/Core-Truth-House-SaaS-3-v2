...
import { KeyRound, Shield, Cpu, SlidersHorizontal } from "lucide-react";
      ...
      <section className="rounded-2xl border border-[var(--cth-admin-border)] bg-white p-6 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="mt-1 text-3xl font-semibold text-[var(--cth-admin-ink)]">API & AI Controls</h2>
          </div>
        </div>
        <p className="mt-3 max-w-2xl text-sm text-[var(--cth-admin-muted)]">
          Centralize platform API configuration, provider oversight, and future AI governance controls inside the new separated admin shell.
        </p>
      </section>

      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard icon={KeyRound} label="API Surface" value="Ready" tone="rose" />
        <StatCard icon={Cpu} label="Provider Controls" value="Active" tone="plum" />
        <StatCard icon={SlidersHorizontal} label="Governance Layer" value="Next" tone="gold" />
        <StatCard icon={Shield} label="Admin Ownership" value="Active" tone="green" />
      </section>

      <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <PanelCard
          title="Platform API Configuration"
          subtitle="Global provider configuration and platform-level API behavior"
        >
          <AdminApiConfig apiConfigs={[]} fetchApiConfigs={() => {}} adminId={adminId} />
        </PanelCard>

        <div className="space-y-4">
          <PanelCard
            title="AI Key Governance"
            subtitle="Personal key controls are intentionally paused"
          >
            <div className="grid gap-4">
              <div className="rounded-xl border border-[var(--cth-admin-border)] bg-[var(--cth-admin-panel)] p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold text-[var(--cth-admin-ink)]">Personal AI API Keys</div>
                    <div className="mt-1 text-xs text-[var(--cth-admin-muted)]">
                      This area stays disabled until the separated admin backend exposes a stable personal-keys contract.
                    </div>
                  </div>
              ...
              <div className="rounded-xl border border-dashed border-[var(--cth-admin-border)] bg-[var(--cth-admin-panel)] p-4">
                <div className="text-sm font-semibold text-[var(--cth-admin-ink)]">Future Controls</div>
                <div className="mt-2 text-sm text-[var(--cth-admin-muted)]">
                  Future home for provider-specific key governance, model access rules, quota oversight, and admin review controls.
                </div>
              </div>
            </div>
          </PanelCard>
          ...
          >
            <div className="grid gap-3">
              <div className="rounded-xl border border-[var(--cth-admin-border)] bg-[var(--cth-admin-panel)] p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold text-[var(--cth-admin-ink)]">Provider Visibility</div>
                    <div className="mt-1 text-xs text-[var(--cth-admin-muted)]">
                      Review platform-level providers, model routing, and high-level config state.
                    </div>
                  </div>
              ...
              <div className="rounded-xl border border-[var(--cth-admin-border)] bg-[var(--cth-admin-panel)] p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold text-[var(--cth-admin-ink)]">Model Governance</div>
                    <div className="mt-1 text-xs text-[var(--cth-admin-muted)]">
                      Future controls for provider rules, model availability, and admin-level safety defaults.
                    </div>
                  </div>
